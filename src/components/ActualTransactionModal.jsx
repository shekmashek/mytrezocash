...
const ActualTransactionModal = ({ isOpen, onClose, onSave, onDelete, editingData, type }) => {
  const { state } = useBudget();
  const { categories, tiers, settings } = state;
  ...
  const availableCategories = type === 'payable' ? categories.expense : categories.revenue;
  const availableTiers = tiers.filter(t => t.type === (type === 'payable' ? 'fournisseur' : 'client'));
  ...
  const handleDeleteClick = () => { if (editingData) { onDelete(editingData.id); } };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-xl font-semibold text-gray-900">{editingData ? `Modifier la ${currentConfig.title}` : `Nouvelle ${currentConfig.title}`}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-between items-center pt-4 border-t">
            ...
            <div className="flex gap-3"><button type="button" onClick={onClose} className="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium">Annuler</button><button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium flex items-center gap-2"><Save className="w-4 h-4" /> {editingData ? 'Modifier' : 'Enregistrer'}</button></div>
          </div>
        </form>
      </div>
    </div>
