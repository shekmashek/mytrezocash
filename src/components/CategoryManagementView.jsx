...
const CategoryManagementView = () => {
  ...
  const handleDeleteSubCategory = (type, mainId, subId) => dispatch({ type: 'DELETE_SUB_CATEGORY', payload: { type, mainId, subId } });
          ...
          <div key={mainCat.id} className="bg-white p-4 rounded-lg shadow-sm border">
            ...
            <ul className="mt-3 space-y-2">
              {mainCat.subCategories.map(subCat => (
                <li key={subCat.id} className="flex items-center justify-between p-2 rounded-md hover:bg-gray-50">
                  ...
                  <div className="flex items-center gap-2">
                    {editingSubCategory?.subId === subCat.id ? (<><button onClick={handleSaveEdit} className="p-1 text-green-600 hover:text-green-800"><Save className="w-4 h-4" /></button><button onClick={handleCancelEdit} className="p-1 text-gray-500 hover:text-gray-700"><X className="w-4 h-4" /></button></>) : (<><button onClick={() => handleStartEdit(type, mainCat.id, subCat.id, subCat.name)} className="p-1 text-blue-600 hover:text-blue-800"><Edit className="w-4 h-4" /></button><button onClick={() => handleDeleteSubCategory(type, mainCat.id, subCat.id)} disabled={isSubCategoryUsed(subCat.name)} className="p-1 text-red-600 hover:text-red-800 disabled:text-gray-300 disabled:cursor-not-allowed" title={isSubCategoryUsed(subCat.name) ? "Suppression impossible: catégorie utilisée" : "Supprimer"}><Trash2 className="w-4 h-4" /></button></>)}
                  </div>
                </li>
              ))}
            </ul>
