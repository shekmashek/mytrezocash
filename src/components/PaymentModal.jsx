...
const PaymentModal = ({ isOpen, onClose, onSave, actualToPay, type }) => {
  ...
  const handleSubmit = (isFinalPayment) => {
    ...
    if (amount > remainingAmount && !isFinalPayment) { if (!window.confirm('Le montant est supérieur au montant restant. Continuer ?')) return; }
    if (!cashAccount) { alert('Veuillez sélectionner un compte de trésorerie.'); return; }
    onSave({ paymentDate, paidAmount: amount, cashAccount, isFinalPayment });
  };
